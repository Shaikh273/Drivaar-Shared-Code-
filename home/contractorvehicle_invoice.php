<?php
    ...
    if((isset($_SESSION['permissioncode']) && $_SESSION['permissioncode'][$page_id]==1) || (isset($_SESSION['adt']) && $_SESSION['adt']==1))
    {
        ...
        $cntquery = "SELECT * FROM `tbl_contractor` WHERE `id`=".$_SESSION['cid'];
                                            ...
                                            <div class='row widget-header widget-header-large' style="padding-right: 50px;padding-left: 0px;padding-bottom: 15px; margin-left: 0px; margin-right: 0px; padding-top: 10px;">
                                                ...
                                                <div class="col-lg-2">
                                                 ...
                                                 <?php
                                                    ...
                                                    $duedate = date("d/m/Y",strtotime($result['duedate']));
                                                ?>
                                                ...
                                                <div class="col-lg-3 mt-3">
                                                        ...
                                                        <span class='invoice-info-label lable-header'><?php echo $result['week_no'];?></span> 
                                                        ...
                                                        <div class='col-sm-5'>
                                                            <div>
                                                                <ul class='list-unstyled spaced'>
                                                                    ...
                                                                    <li><?php echo $result1['address'];?></li>
...
<tbody>
    ...
    $tblquery = "SELECT c.*,v.`registration_number` FROM `tbl_contractorinvoice`c 
                ...
                WHERE c.`isdelete`=0 AND c.`isactive`=0 AND c.`week_no`=".$result['week_no']." AND c.cid = ".$result['cid']." AND c.istype =3 ORDER BY c.`from_date` asc";
    ...
    while($tblresult = mysqli_fetch_array($tblrow))
    {
        ...
        for($i = $begin; $i <= $end; $i->modify('+1 day'))
        {
            ...
            $qr = "SELECT * FROM `tbl_vehiclerental_agreement` WHERE '".$i->format("Y-m-d")."' between `pickup_date` and `return_date` AND `vehicle_id`=".$tblresult['cid'];
            $qrrow =  $mysql -> selectFreeRun($qr);
            $qrresult = mysqli_fetch_array($qrrow);

            $adddate = date("d/m/Y",strtotime($i));
            $total = $qrresult['price_per_day'];
            ?>
            <tr>
                <td><?php echo 'Van hire ['.$tblresult['registration_number'].'] - '.$i->format("Y-m-d"); ?></td>
                <td>1</td>
                <td>Each</td>
                <td><?php echo '£ '.$qrresult['price_per_day']; ?></td>
    ...
    ?>
                                                    ...
                                                    <div class='row'>
                                                        <div class='col-sm-7 pull-left'></div>
                                                            <div class='col-sm-5 pull-right' style="text-align: right;"><br><br>
