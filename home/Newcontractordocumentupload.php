...
foreach($_POST['type'] as $key => $value) {
  echo "POST parameter '$key' has '$value'".$_POST['type']."<br>";
  $value[$i]['type']=$_POST['type'];
  $value[$i]['contractor_id']=$_POST['cid'];
  $value[$i]['name']=$_POST['name'];
  if($_POST['expiredate'])
  {
  	$value[$i]['expiredate']=$_POST['expiredate'];
  }
  $i++;
}
...
// $uploadDir = 'uploads/contractordocument/'; 
// $uploadedFile = ''; 

// $fileName = basename($_FILES["file"]["name"]); 
// if(!empty($fileName))
// { 
//     $targetFilePath = $uploadDir . $_POST['cid'].'_'.$fileName; 
...
//                 $valus[0]['file'] = $_POST['cid'].'_'.$uploadedFile;
//                 $status = 1; 
//             }
//             else
//             { 
