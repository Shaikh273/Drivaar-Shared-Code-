...
if (isset($_POST['action']) && $_POST['action'] == 'ownerform') {
    ...
    if ($type == 1) {
        ...
        if (!empty($_POST['invoice_email'])) {
            $valus[0]['invoice_email'] = $_POST['invoice_email'];
            $makecol[$flcnt] = 'invoice_email';
            $flcnt++;
        }

        if (!empty($_POST['company'])) {
            $valus[0]['company'] = $_POST['company'];
            $makecol[$flcnt] = 'company';
            $flcnt++;
        }
        $depot =  $mysql->selectWhere('tbl_contractor', 'id', '=', $_POST['company'], 'int');
        $dptresult = mysqli_fetch_array($depot);
        if (!empty($dptresult['name'])) {
            $valus[0]['company_name'] = $dptresult['name'];
            $makecol[$flcnt] = 'company_name';
            $flcnt++;
        }
        if (!empty($_POST['daily_rate'])) {
            $valus[0]['daily_rate'] = $_POST['daily_rate'];
            $makecol[$flcnt] = 'daily_rate';
            $flcnt++;
        }
        if (!empty($_POST['daily_miles_rate'])) {
            $valus[0]['daily_miles_rate'] = $_POST['daily_miles_rate'];
            $makecol[$flcnt] = 'daily_miles_rate';
            $flcnt++;
        }
        if (!empty($_POST['start_date'])) {
            $valus[0]['start_date'] = $_POST['start_date'];
            $makecol[$flcnt] = 'start_date';
            $flcnt++;
        }
        if (!empty($_POST['leave_date'])) {
            $valus[0]['leave_date'] = $_POST['leave_date'];
            $makecol[$flcnt] = 'leave_date';
            $flcnt++;
        }
        if (!empty($_POST['dob'])) {
            $valus[0]['dob'] = $_POST['dob'];
        ...
        if (!empty($_POST['ni_number'])) {
            $valus[0]['ni_number'] = $_POST['ni_number'];
            $makecol[$flcnt] = 'ni_number';
            $flcnt++;
        }
        if (!empty($_POST['employee_id'])) {
            $valus[0]['employee_id'] = $_POST['employee_id'];
            $makecol[$flcnt] = 'employee_id';
            $flcnt++;
        }
    } else {
        if (!empty($_POST['company_reg'])) {
            $valus[0]['company_reg'] = $_POST['company_reg'];
            $makecol[$flcnt] = 'company_reg';
            $flcnt++;
        }
        if (!empty($_POST['vat_number'])) {
            $valus[0]['vat_number'] = $_POST['vat_number'];
            $makecol[$flcnt] = 'vat_number';
            $flcnt++;
        }
        if (!empty($_POST['bank_name'])) {
            $valus[0]['bank_name'] = $_POST['bank_name'];
            $makecol[$flcnt] = 'bank_name';
            $flcnt++;
        }
        if (!empty($_POST['account_number'])) {
            $valus[0]['account_number'] = $_POST['account_number'];
            $makecol[$flcnt] = 'account_number';
            $flcnt++;
        }
        if (!empty($_POST['sort_code'])) {
            $valus[0]['sort_code'] = $_POST['sort_code'];
            $makecol[$flcnt] = 'sort_code';
            $flcnt++;
        }
        if (!empty($_POST['street_address'])) {
            $valus[0]['street_address'] = $_POST['street_address'];
            $makecol[$flcnt] = 'street_address';
            $flcnt++;
        }
        if (!empty($_POST['postcode'])) {
            $valus[0]['postcode'] = $_POST['postcode'];
            $makecol[$flcnt] = 'postcode';
            $flcnt++;
        }
    ...
    if (isset($_POST['insert'])) {
        ...
        $makecol = array('contractor_id', 'type', 'starts', 'end', 'description', 'update_date');
    ...
    $query = "SELECT DISTINCT ci.id as ciid, ci.*,c.name as cname,c.email as cemail FROM `tbl_contractorinvoice` ci INNER JOIN `tbl_contractor` c ON c.id=ci.cid WHERE ci.`isdelete`=0 AND c.isdelete=0 AND ci.id=" . $id;
    $row =  $mysql->selectFreeRun($query);
    ...
    if ($statusinsert) {
        ...
        $title = 'Insert';
        $message = 'Data has been inserted successfully.';
        $expquery =  "SELECT DISTINCT v.*,vs.name as sname FROM `tbl_vehiclerental_agreement` va RIGHT JOIN `tbl_vehicles` v ON v.id = va.vehicle_id INNER JOIN `tbl_vehiclesupplier` vs ON vs.`id`=v.`supplier_id` WHERE v.depot_id=$sdpid AND v.status=4 AND v.isdelete=0 AND v.`supplier_id` iS NOT NULL ORDER BY v.`id` ASC";
        $exprow =  $mysql->selectFreeRun($expquery);
