...
include 'DB/config.php';
    $page_title = "Contractor Timesheet";
    $page_id=7;
    if(!isset($_SESSION)) 
    {
        session_start();
    ...
    if((isset($_SESSION['permissioncode']) && $_SESSION['permissioncode'][$page_id]==1) || (isset($_SESSION['adt']) && $_SESSION['adt']==1))
    {
        ...
        $complete = $cntresult['iscomplated'];
...
<head>
    ...
    $(document).ready(function() 
    {
        ...
        for(j=1;j<8;j++)
        {
          showFlag = showdata(document.getElementById("hiddendate-"+j));
            if(showFlag == 1)
            {
              atndc++;
            }
        }
        $("#attendance").html(atndc);
    ...
    function showdata(data)
    {
        ...
        $.ajax({
                ...
                // async: false,
                dataType:"JSON",            
                data: {action: 'ContractorTimesheetData',uniqid:allInOne,userid:cid,showDate:showDate},
    ...
    function insertTimesheet(data,userid,rateid,number,ischeckval)
    {
        ...
        var uniqnumber = date+'-'+rateid+'-'+cid;
        $.ajax({
            url: "InsertData.php", 
            type: "POST", 
            dataType:"JSON",            
            data: {action: 'ContractorTimesheetForm',date:date,rateid:rateid,uniqid:uniqnumber,value:value,cid:cid,userid:userid,ischeckval:ischeckval},
            success: function(data) 
            {
                if(data.status==1)
                {
    ...
    function setdate(date1)
    {
            ...
            var result = Math.ceil(( currentdate.getDay() + 1 + numberOfDays) / 7);
            var week = [];
            var set = ['<th style="width: 35%; vertical-align: middle;padding-left: 20px;" class="border-right bg-white"><span class="align-items-right col-md-2"><button  class="btn btn-secondary" onclick="PreviousWeek();">Previous</button><button  class="btn btn-secondary" onclick="NextWeek();">Next</button></span><a href="contractor_invoice.php?invkey=" class="btn btn-secondary btn- ml-2" small="small" target="_blank" id="setweek">View Invoice for Week '+result+'</a></th>'];
    ...
    function getInvcKey(cid,cdate,utype)
    {
        $.ajax({
              type: "POST",
              url: "loaddata.php",
              async: false, 
              data: {action : 'getInvcKey',id:cid,cdate:cdate,utype:utype},
              dataType: 'JSON',
              success: function(data) {
                $('#setweek').attr("href", "contractor_invoice.php?invkey="+data.invcKey);
...
<body class="skin-default-dark fixed-layout">
    <div id="iframeloading" style= "display: none; position: absolute; top: 0px; left: 0px; width: 100%; height: 100%;">
     <img src="loader.gif" alt="loading" style="top: 50%; position: relative; left: 50%;"  />
</div> 
                        ...
                        <div class="card-body">
                           ...
                           <div class="row">
                            <div class="col-md-3"></div>
                                ...
                                <button class='today-date btn btn-outline-secondary' id="todaydate">Today</button>
  ...
  <div class="col-md-4">
        <div class="row">
            <div class="card datacard col-md-5" style="margin-top: 110px;">
              ...
              <div class="card-body">
                <h5 class="m-0" style="font-weight: 700;" id="attendance">0</h5>
              </div>
            </div>
            <div class="card datacard col-md-5" style="margin-top: 110px;">
              <div class="card-header dataheader">
              ...
              <div class="card-body">
                <h5 class="m-0" style="font-weight: 700;" id="weekTotalAmount">£ 0</h5>
              </div>
            </div>
        </div>
        <div class="row">
    ...
    <table class="table w-100 table-hover table-sm table-vcente" small="small" centered="centered" hover="hover" >
        <thead>
            <tr id="setdates">
                <th style="width: 35%; vertical-align: middle;padding-left: 20px;" class="border-right bg-white">
                 <span class="align-items-right col-md-6">
                    ...
                    <?php  //$invkey=base64_encode($id."#1#".$typeresult["week_no"]."#".$typeresult["weekyear"]); ?>
                ...
                <?php
                    ...
                    for($i=1;$i<=7;$i++)
                    {
                        ...
                        <th class="text-center">
                            <input type="hidden" id="hiddendate-<?php echo $i;?>" name="hiddendate-<?php echo $i;?>" class="th" onchange="showdata(this);">
        ...
        <tbody>
            ...
            <?php
                    $mysql = new Mysql();
                    $mysql -> dbConnect();
                    $query = "SELECT * FROM `tbl_paymenttype` WHERE `isdelete`= 0 AND `isactive`= 0 AND `type`=1 AND `applies`=1 AND (`depot_id` LIKE '".$cntresult['depot'].",%' OR `depot_id` LIKE '%,".$cntresult['depot']."' OR `depot_id` LIKE '".$cntresult['depot']."' OR `depot_id` LIKE '%,".$cntresult['depot'].",%')";
                    ...
                    for($j=1;$j<=7;$j++)
                    {
                        if($result['paymentperstop']==1)
                        {
                            ...
                            <td class="text-center border">
                                <span class="custom-control custom-checkbox">
                                    <label class="custom-control custom-checkbox m-b-0">
<input type="checkbox" id="<?php echo $result['id']."-".$id; ?>" class="custom-control-input td-<?php echo $j;?>  allCheckField" onchange="insertTimesheet(this,<?php echo $userid?>,<?php echo $result['id']?>,<?php echo $j?>,1);" value="1">
                        ...
                        else
                        {
                            ?>
                            <td class="text-center border">
<input type="text" class="border rounded text-center td-<?php echo $j;?> allTextField" id="<?php echo $result['id']."-".$id; ?>" style="width:60px" wire:model="values.59.2021-04-25" onfocusout="insertTimesheet(this,<?php echo $userid?>,<?php echo $result['id']?>,<?php echo $j?>,2);">
                            </td>
                            <?php
                        }
                        ?>     
            ...
            <?php
                    $mysql = new Mysql();
                    $mysql -> dbConnect();
                    $query = "SELECT * FROM `tbl_paymenttype` WHERE `isdelete`= 0 AND `isactive`= 0 AND `type`=2 AND `applies`=1 AND (`depot_id` LIKE '".$cntresult['depot'].",%' OR `depot_id` LIKE '%,".$cntresult['depot']."' OR `depot_id` LIKE '".$cntresult['depot']."' OR `depot_id` LIKE '%,".$cntresult['depot'].",%')";
                    $raterow =  $mysql -> selectFreeRun($query);
                    while($result = mysqli_fetch_array($raterow))
                    {
                        $blocktime='';
...
<input type="text" class="border rounded text-center td-<?php echo $j;?>" id="<?php echo $result['id']."-".$id; ?>" style="width:60px" wire:model="values.59.2021-04-25" onfocusout="insertTimesheet(this,<?php echo $userid?>,<?php echo $result['id']?>,<?php echo $j?>,2);">
            ...
            <?php
                $mysql = new Mysql();
                $mysql -> dbConnect();
                $query = "SELECT * FROM `tbl_paymenttype` WHERE `isdelete`= 0 AND `isactive`= 0 AND `type`=3 AND `applies`=1 AND (`depot_id` LIKE '".$cntresult['depot'].",%' OR `depot_id` LIKE '%,".$cntresult['depot']."' OR `depot_id` LIKE '".$cntresult['depot']."' OR `depot_id` LIKE '%,".$cntresult['depot'].",%')";
                $raterow =  $mysql -> selectFreeRun($query);
                while($result = mysqli_fetch_array($raterow))
                {
                    $blocktime='';
...
<input type="text" class="border rounded text-center td-<?php echo $j;?>" id="<?php echo $result['id']."-".$id; ?>" style="width:60px" wire:model="values.59.2021-04-25" onfocusout="insertTimesheet(this,<?php echo $userid?>,<?php echo $result['id']?>,<?php echo $j?>,2);">
    ...
    else
    {
        ...
        <tbody>
            <tr>
                <td colspan="8" style="text-align: center;font-size: 17px;"><br>Please Fill Your All Onboarding Task.</td>
            </tr>
        </tbody>
        <?php
    }
