...
if (isset($_POST['action']) && $_POST['action'] == 'loadpaymenttabledata') {
    ...
    while ($userresult = mysqli_fetch_array($userrow)) {
        ...
        if ((isset($_SESSION['permissioncode']) && $_SESSION['permissioncode'][37] == 1) || (isset($_SESSION['adt']) && $_SESSION['adt'] == 1)) {
            if ($userresult['ispasswordchange'] == 0) {
                $showurl = "<a href='#' style='color: #405efb;' onclick=\"resendurl('" . $userresult['id'] . "')\" data-toggle='tooltip' title='Resend Invitaion'><span><i class='fas fa-sync fa-lg'></i></span></a> ";
            } else {
                $showurl = '';
            }
    ...
    if ($status == "25") {
        ...
        $query = "SELECT DISTINCT v.*,DATE_FORMAT(v.`insert_date`,'%D %M%, %Y') as date1,vs.`name` as statusname,vs.`colorcode` ,vsp.`name` as suppliername,vg.`name` as groupname,vm.`name` as makename,vt.`name` as typename,vmo.`name` as modelname,vfi.`insurance_company` as insname, dpt.`name` as dpt_name, avo.vehicle_ownername as vehicle_owner
        FROM `tbl_vehicles` v 
        INNER JOIN tbl_workforcedepotassign w ON v.`depot_id` IN (w.`depot_id`) AND w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL
        LEFT JOIN `tbl_vehiclestatus` vs ON vs.`id`=v.`status` 
        LEFT JOIN `tbl_vehiclesupplier` vsp ON vsp.`id`=v.`supplier_id` 
    ...
    while ($typeresult = mysqli_fetch_array($vehiclesrow)) {
        ...
        $data[] = [
            ...
            'dpt_name' => $typeresult['dpt_name'],
            'insurance' => $typeresult['insname'],
            'group_id' => $typeresult['groupname'],
            'owner_id' => $typeresult['vehicle_owner'],
...
} else if (isset($_POST['action']) && $_POST['action'] == 'loadwalletcategorytypetabledata') {
    ## Search 
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (name LIKE '%" . $searchValue . "%' or insert_date LIKE '%" . $searchValue . "%') ";
    ...
    $sql1 = "select count(*) as allcount from   tbl_walletcategory where isdelete = 0";
    $sel =  $mysql->selectFreeRun($sql1);
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
    $sql2 = "select count(*) as allcount from   tbl_walletcategory WHERE isdelete = 0 " . $searchQuery;
    $sel =  $mysql->selectFreeRun($sql2);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];
    $ord = "";

    $query = "SELECT *, DATE_FORMAT(`insert_date`,'%D %M%, %Y') as date1 FROM `tbl_walletcategory` WHERE `isdelete`= 0" . $searchQuery . " order by " . $columnName . " " . $columnSortOrder . ", id desc limit " . $row . "," . $rowperpage;
    $typerow =  $mysql->selectFreeRun($query);
    $data = array();
    while ($typeresult = mysqli_fetch_array($typerow)) {
        if ($typeresult['isactive'] == 0) {
        ...
        $statustd = "<div id='" . $typeresult['id'] . "-td'>
                        <button type='button' class='isactivebtn btn btn-" . $statuscls . "' 
                        onclick=\"Isactivebtn('" . $typeresult['id'] . "','" . $typeresult['isactive'] . "','Walletcategorytypeisactive')\">" . $statusname . "</button>
                    </div>";
...
// }
    ...
    while ($userresult = mysqli_fetch_array($userrow)) {
        ...
        if ((isset($_SESSION['permissioncode']) && $_SESSION['permissioncode'][5] == 1) || (isset($_SESSION['adt']) && $_SESSION['adt'] == 1)) {
            // $name = "<a href='contractorDetails.php?cpcid=".base64_encode($userresult['id'])."'><span  style='color: blue;'>" . $userresult['name'] . "</span></a>";
        ...
        if ((isset($_SESSION['permissioncode']) && $_SESSION['permissioncode'][7] == 1) || (isset($_SESSION['adt']) && $_SESSION['adt'] == 1)) {
            if ($userresult['ispasswordchange'] == 0 && $userresult['isactive'] == 0) {
                $showurl = "<a href='#' style='color: #405efb;' onclick=\"resendurl('" . $userresult['id'] . "')\" data-toggle='tooltip' title='Resend Invitaion For Password'><span><i class='fas fa-sync fa-lg' style='color: orange;'></i></span></a> ";
            } else {
                $showurl = '';
            }
        }
        ...
        if ((isset($_SESSION['permissioncode']) && $_SESSION['permissioncode'][7] == 1) || (isset($_SESSION['adt']) && $_SESSION['adt'] == 1)) {
            if ($userresult['ispasswordchange'] == 0 && $userresult['isactive'] == 2) {
                $showonboardurl = " <a href='#' style='color: #405efb;' onclick=\"resendonboardurl('" . $userresult['id'] . "')\" data-toggle='tooltip' title='Resend Invitaion'><span><i class='fas fa-sync fa-lg'></i></span></a> ";
            } else {
                $showonboardurl = '';
            }
    ...
    $query = "SELECT *, DATE_FORMAT(`insert_date`,'%D %M%, %Y') as insertdate1,DATE_FORMAT(`date`,'%D %M%, %Y') as date1 FROM `tbl_contractorattendance` WHERE `isdelete`= 0 AND contractor_id=" . $cid . $searchQuery . " order by id desc limit " . $row . "," . $rowperpage;
    $typerow =  $mysql->selectFreeRun($query);
    $data = array();
    while ($typeresult = mysqli_fetch_array($typerow)) {
        ...
        $data[] = [
            ...
            'end' => $typeresult['end'],
            'Duration' => $duration,
            'insertdate' => $typeresult['insertdate1'],
            'Status' => $statustd,
            'Action' => $delete
        ];
    }
    ...
    while ($typeresult = mysqli_fetch_array($typerow)) {
        ...
        <i class='fas fa-eye fa-lg'></i></a>";
        $checkDir = explode("/", $typeresult['file'])[0];
        if ($checkDir == "documents") {
            $view = "<a target = '_blank' href='" . $webroot . 'contractorEmailRegistration/' . $typeresult['file'] . "' class='adddata'>
        <i class='fas fa-eye fa-lg'></i></a>";
        }
    ...
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
    $sql2 = "select count(*) as allcount from tbl_contractorinvoice WHERE isdelete = 0 AND `istype`=1 AND cid=" . $cid . $searchQuery;
    $sel =  $mysql->selectFreeRun($sql2);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];
    $ord = "";

    $query = "SELECT DISTINCT *, DATE_FORMAT(`duedate`,'%D %M%, %Y') as duedate, DATE_FORMAT(`update_date`,'%D %M%, %Y') as updatedate FROM `tbl_contractorinvoice` WHERE `istype`=1 AND `isdelete`= 0 AND `cid`=" . $cid . " order by `week_no` desc limit " . $row . "," . $rowperpage;
    $typerow =  $mysql->selectFreeRun($query);
    $data = array();
    while ($typeresult = mysqli_fetch_array($typerow)) {
        $vat = '';
        ...
        $vat = "<div id='" . $typeresult['id'] . "-td'>
                    <button type='button' class='isactivebtn btn btn-" . $statuscls . "' 
                    onclick=\"Isactivevat('" . $typeresult['id'] . "','" . $typeresult['vat'] . "','NewvalidateVat')\">" . $statusname . "</button>
                </div>";
        ...
        $fromdate = date("d M, Y", strtotime($week_array['week_start'] . '-1 day'));
        $todate = date("d M, Y", strtotime($week_array['week_end'] . '-1 day'));
    ...
    $searchQuery = " ";
    if ($searchValue != '') {
        $searchQuery = " and (CAST(c.`invoice_no` as VARCHAR(6)) LIKE '%" . $searchValue . "%' or CAST (c.`week_no` AS VARCHAR(30)) LIKE '%" . $searchValue . "%' or c.`insert_date` LIKE '%" . $searchValue . "%') ";
    }
    $mysql = new Mysql();
    $mysql->dbConnect();
    ...
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
    $sql2 = "select count(*) as allcount from tbl_contractorinvoice WHERE isdelete = 0 AND `istype`=2 AND cid=" . $wid . $searchQuery;
    $sel =  $mysql->selectFreeRun($sql2);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];
    $ord = "";

    $query = "SELECT c.*, DATE_FORMAT(c.`duedate`,'%D %M%, %Y') as duedate, DATE_FORMAT(c.`update_date`,'%D %M%, %Y') as updatedate,u.`vat_number` as vat_number1  
    FROM `tbl_contractorinvoice` c 
    Inner join `tbl_user` u ON u.`id`=c.`cid`
    WHERE c.`istype`=2 AND c.`isdelete`= 0 AND c.`cid`=" . $wid . " order by c.`id` desc limit " . $row . "," . $rowperpage;
    //echo $query;
    $typerow =  $mysql->selectFreeRun($query);
    $data = array();
    while ($typeresult = mysqli_fetch_array($typerow)) {
        $vat = '';
        if ($typeresult['vat_number1'] > 0 || $typeresult['vat_number1'] != '') {
            $vat = '<i class="fas fa-check fa-lg edit"></i>';
        }
        $invoicestatus = $typeresult['status_id'];
        $week_array = getStartAndEndDate($typeresult['week_no'], $typeresult['weekyear']);
    ...
    $sql1 = "SELECT count(DISTINCT i.`id`) as allcount 
            ...
            INNER JOIN tbl_workforcedepotassign w ON w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL
            WHERE i.`depot_id` IN (w.depot_id) AND i.`istype`=2 AND i.`isdelete`= 0 AND i.`depot_id` LIKE '" . $depot . "'  $vat  AND i.`status_id` LIKE '" . $statusid . "'";
    $sel =  $mysql->selectFreeRun($sql1);
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
    ...
    $sql2 = "SELECT count(DISTINCT i.`id`) as allcount 
            ...
            INNER JOIN `tbl_user` c ON c.`id`=i.`cid` 
            INNER JOIN tbl_workforcedepotassign w ON w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL
            WHERE i.`depot_id` IN (w.depot_id) AND i.`istype`=2 AND i.`isdelete`= 0 AND i.`depot_id` LIKE '" . $depot . "'  $vat  AND i.`status_id` LIKE '" . $statusid . "'" . $searchQuery . " " . $extraquery . " " . $wekNo;
    ...
    $query = "SELECT DISTINCT i.*,(SELECT `name` FROM `tbl_depot` WHERE `id`=i.`depot_id`) as depotname, DATE_FORMAT(i.`duedate`,'%D %M%, %Y') as duedate,DATE_FORMAT(i.`update_date`,'%D %M%, %Y') as updatedate,c.`name` as cntname, c.`vat_number`  
             FROM `tbl_contractorinvoice` i  INNER JOIN `tbl_user` c ON c.`id`=i.`cid`  INNER JOIN tbl_workforcedepotassign w ON w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL WHERE i.`depot_id` IN (w.depot_id) AND i.`istype`=2  AND i.`isdelete`= 0 AND i.`depot_id` LIKE '" . $depot . "'  $vat  AND i.`status_id` LIKE '" . $statusid . "'" . $searchQuery . " " . $extraquery . " " . $wekNo . " order by i.`id` desc limit " . $row . "," . $rowperpage;
    //echo $query;
    $typerow =  $mysql->selectFreeRun($query);
    ...
    while ($typeresult = mysqli_fetch_array($typerow)) {
        $vat = '';
        if ($typeresult['vat_number'] != '') {
            $vat = '<i class="fas fa-check fa-lg edit"></i>';
        }
        // if (isset($typeresult['vat_number']) || !empty($typeresult['vat_number'])) {
        //     $vat = '<i class="fas fa-check fa-lg edit"></i>';
        // }
        ...
        //     'total' => '<b>£ ' . $totalshow . '</b>',
        //     'status' => "<span class='label label-secondary' style='color: " . $stresult['color'] . ";background-color: " . $stresult['backgroundcolor'] . "'><b>" . $statusname . "</b></span>",
        //    // 'period'=>'<b>Wk #'.$typeresult['week_no'].'</b><br> <small>'.$fromdate.' >> '.$todate.'</small>',
        //     'invoice_no' => $typeresult['invoice_no'],
        //     'vat' => $vat,
        //     'due' => $due,
        //     'Action' => $action
        // ];
        $data[] = [
            'bulkOption' => "<input type='checkbox' class='check' data-checkbox='icheckbox_flat-red' style='width:40%' value='" . $typeresult['invoice_no'] . "' onchange='selectInvoiceNo(this);' $checked>",
            'name' => $typeresult['cntname'] . '<br><small>' . $typeresult['depotname'] . '</small>',
            'total' => '<b>£ ' . $totalshow . '</b>',
            'status' => "<span class='label label-secondary' style='color: " . $stresult['color'] . ";background-color: " . $stresult['backgroundcolor'] . "'><b>" . $statusname . "</b></span>",
            'period' => '<b>Wk #' . $typeresult['week_no'] . '</b><br> <small>' . $fromdate . ' >> ' . $todate . '</small>',
    ...
    $sql1 = "SELECT count(DISTINCT i.`id`) as allcount 
            ...
            INNER JOIN tbl_workforcedepotassign w ON w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL
            WHERE i.`depot_id` IN (w.depot_id) AND i.`istype`=1 AND i.`isdelete`= 0 AND i.`depot_id` LIKE '" . $depot . "' $vat AND i.`status_id` LIKE '" . $statusid . "'";
    $sel =  $mysql->selectFreeRun($sql1);
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];
    ...
    $sql2 = "SELECT count(DISTINCT i.`id`) as allcount 
            ...
            INNER JOIN `tbl_contractor` c ON c.`id`=i.`cid` 
            INNER JOIN tbl_workforcedepotassign w ON w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL
            WHERE i.`depot_id` IN (w.depot_id) AND i.`istype`=1 AND i.`isdelete`= 0 AND i.`depot_id` LIKE '" . $depot . "' $vat AND i.`status_id` LIKE '" . $statusid . "'" . $searchQuery . " " . $extraquery . " " . $wekNo;
    ...
    $query = "SELECT DISTINCT i.*,(SELECT `name` FROM `tbl_depot` WHERE `id`=i.`depot_id`) as depotname,DATE_FORMAT(i.`duedate`,'%D %M%, %Y') as duedate, DATE_FORMAT(i.`update_date`,'%D %M%, %Y') as updatedate,c.`name` as cntname, c.`vat_number` 
        FROM `tbl_contractorinvoice` i INNER JOIN `tbl_contractor` c ON c.`id`=i.`cid`
            INNER JOIN tbl_workforcedepotassign w ON w.`wid` LIKE ('" . $userid . "') AND w.`isdelete`=0 AND w.`isactive`=0 AND w.`release_date` IS NULL
            WHERE i.`depot_id` IN (w.depot_id) AND i.`istype`=1 AND i.`isdelete`= 0 AND i.`depot_id` LIKE '" . $depot . "' $vat AND i.`status_id` LIKE '" . $statusid . "'" . $searchQuery . " " . $extraquery . " " . $wekNo . " order by i.`id` desc limit " . $row . "," . $rowperpage;
...
//bhumika changes
    ...
    $sql2 = "select count(*) as allcount from tbl_conditionalreportdata WHERE isdelete = 0 AND `isactive`=0 AND vehicle_id=" . $vid . $searchQuery;
    $sel =  $mysql->selectFreeRun($sql2);
    $records = mysqli_num_rows($sel);
    ...
    WHERE `isdelete`=0 AND `isactive`=0 AND `vehicle_id`=" . $vid . " " . $searchQuery . "
    ...
    while ($typeresult = mysqli_fetch_array($typerow)) {
        ...
        AND `vehicle_id`=" . $vid . "  AND `state`=0";
        ...
        <a href='damagereportpdf.php?id=" . $typeresult["id"] . "&vid=" . $vid . "' class='edit' data-toggle='tooltip' title='View'><span><i class='fas fa-eye fa-lg'></i></span></a>";

        $date1 = date("dS F, Y H:i", strtotime($typeresult['insert_date']));

        $query5 = "SELECT name FROM `tbl_user` WHERE `id`=" . $typeresult['userid'];
