...
class InvoiceAmountClass
{
	...
	function ContractorInvoiceTotal($ginvID)
	{
		...
		$lasttotal = $finaltotal - $paidamount;
		$finaltotal = round($lasttotal, 2);
		...
		$totals['totalnet'] = $finalnet;
		$totals['totalvat'] = $finalvat;
		$mysql->dbDisConnect();
		// return  $finaltotal;
		return $totals;
	}

	function ContractorMonthlyInvoiceTotal($ginvID)
	{
		$mysql = new Mysql();
		$mysql->dbConnect();
		$tblquery = "SELECT ci.`id`,ci.month,ci.monthyear,ci.`invoice_no`,ct.`rateid`,ct.`date`,ct.`value`,ct.`ischeckval`,
		ci.`vat` as civat,p.`name`,p.`type`,p.`amount`,p.`vat`,ci.`cid`,ci.`from_date`,ci.`to_date`,c.`vat_number` 
		FROM `tbl_monthlyinvoice` ci 
		INNER JOIN `tbl_contractortimesheet` ct ON ct.cid=ci.cid 
		INNER JOIN `tbl_paymenttype` p ON p.`id`=ct.`rateid` 
		INNER JOIN `tbl_contractor` c ON c.id = ci.cid  
		WHERE ci.`isdelete`=0 AND ci.`isactive`=0 AND ci.`invoice_no`='" . $ginvID . "' 
		AND ct.`date` BETWEEN ci.`from_date` AND ci.`to_date`
		 AND ct.`value` NOT LIKE '0' AND ct.`value` NOT LIKE '' 
		 AND ct.isdelete=0 ORDER BY p.`type` ASC, ct.`date` ASC,ct.`rateid` ASC";
		$tblrow =  $mysql->selectFreeRun($tblquery);
		$finaltotal = 0;
		$totalnet = 0;
		$totalvat = 0;
		...
		while ($tblresult = mysqli_fetch_array($tblrow)) {
			...
			$frm_date3 = $tblresult['from_date'];
			$to_date3 = $tblresult['to_date'];
			$monthNo = $tblresult['month'];
			$monthyear = $tblresult['monthyear'];
		...
		$VehRentQry = "SELECT DISTINCT va.*,v.registration_number 
		FROM `tbl_vehiclerental_agreement` va 
		INNER JOIN `tbl_vehicles` v ON v.id=va.vehicle_id 
		WHERE va.`driver_id`=$cid3 AND (va.`pickup_date` BETWEEN '$frm_date3' AND '$to_date3' OR va.`return_date` 
		BETWEEN '$frm_date3' AND '$to_date3' OR '$frm_date3' BETWEEN va.`pickup_date` AND va.`return_date` 
		OR '$to_date3' BETWEEN va.`pickup_date` AND va.`return_date`) AND va.`iscompalete`=1 
		AND va.`isdelete`=0 AND v.supplier_id>1 ORDER BY va.`pickup_date` ASC";
		$VehRentFire =  $mysql->selectFreeRun($VehRentQry);
		while ($VehRentData = mysqli_fetch_array($VehRentFire)) {
			$pikupD = $VehRentData['pickup_date'];
			if (strtotime($VehRentData['pickup_date']) < strtotime($frm_date3)) {
		...
		$number = str_pad($monthNo, 2, '0', STR_PAD_LEFT);
		$loanweeks = $monthyear . '-' . $number;

		$tquery = "SELECT c.amount FROM `tbl_contractorpayment` c 
			INNER JOIN `tbl_contractorlend` l ON l.`id`=c.`loan_id` 
			WHERE c.`cid`=$cid3 AND c.`week_no` IN ('" . $loanweeks . "') AND c.`isdelete`=0";
		$trow =  $mysql->selectFreeRun($tquery);
		$paidamount = 0;
		while ($tresult = mysqli_fetch_array($trow)) {
			$paidamount += $tresult['amount'];
