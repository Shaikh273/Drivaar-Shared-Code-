...
if($wid)
{
	$query = "SELECT * FROM `tbl_user` WHERE `id`='$wid' AND `isdelete`=0 AND `isactive`=0";
}
else
{
	$query = "SELECT * FROM `tbl_user` WHERE `isdelete`=0 AND `isactive`=0";
}
...
while($result = mysqli_fetch_array($row))
{
	...
	if($result['depot']>0)
	{
		...
		$invoiceqry1 = "SELECT * FROM `tbl_contractorinvoice` WHERE `cid`=".$result['id']." AND `isdelete`=0 AND `isactive`=0 AND `istype`=2 ORDER BY `week_no` DESC LIMIT 1";
        ...
        else
        {
        	...
        	$cntquery = "SELECT week(`date`) AS week_no, year(`date`) As dateyear FROM `tbl_workforcetimesheet` WHERE `wid`=".$result['id']." ORDER BY `date` ASC LIMIT 1";
	        ...
	        if($cntresult = mysqli_fetch_array($cntrow))
	        {
	        	...
	        	while($weekno1<=$currentweek)
	        	{
	        		...
	        		$querychk = "SELECT `to_date` FROM `tbl_contractorinvoice` WHERE `cid`=".$result['id']." AND `week_no`=".$weekno1." AND `istype`=2 ORDER BY `to_date` DECS LIMIT 1";
