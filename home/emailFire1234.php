<?php
		...
		$mysql = new Mysql();
		$db = $mysql -> dbConnect();
		$query="SELECT DISTINCT ci.id as ciid, ci.*,c.name as cname,c.email as cemail FROM `tbl_contractorinvoice` ci INNER JOIN `tbl_contractor` c ON c.id=ci.cid WHERE (ci.`status_id`=7 OR ci.`status_id`=8) AND ci.cid=5 AND ci.`isdelete`=0 AND c.isdelete=0 ORDER BY ci.week_no DESC LIMIT 1";
      ...
      $newDt = date("Y-m-d H:i:s");
	    ...
	    //echo $cnt."<br><br><br> SENT => ".$sent."<br><br><br> FAILED => ".$failed;
?>