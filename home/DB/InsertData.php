...
else if(isset($_POST['action']) && $_POST['action'] == 'ContractorTimesheetForm')
{
    ...
    if($_POST['date'] && $_POST['value'] && $_POST['uniqid'])
    {
        $mysql = new Mysql();
        $mysql -> dbConnect();
...
else if(isset($_POST['action']) && $_POST['action'] == 'msformrentalagreementstep2')
{
    ...
    {
        if($key == 'action' || $key == 'last_insert_id'){
    ...
    // if($clsname == "step_1")  {  
        ...
        $query = "SELECT * FROM `tbl_vehicles` WHERE id =".$_POST['vehicle_id'];
        $row =  $mysql -> selectFreeRun($query); 
        $result = mysqli_fetch_array($row);
        ...
        $response['model_id'] = $result2['id'];
    // }
    // $response['vehicle_id'] = $_POST['vehicle_id'];
    $response['status'] = $status;
    $response['title'] = $title;
    $response['message'] = $message;
    echo json_encode($response); 
...
else if(isset($_POST['action']) && $_POST['action'] == 'BulkDriversform')
{
    ...
    {  
        ...
        foreach ($_POST['driverid'] as $driverid) {
            ...
            $valus[$i]['rota_uniqid'] = 'td_'.$_SESSION['userid'].'_'.$driverid.'_'.$colno.'_'.$_POST['bulkdate'];
        ...
        $extra = "ON DUPLICATE KEY UPDATE `rota_uniqid`='td_".$_SESSION['userid'].'_'.$driverid.'_'.$colno.'_'.$_POST['bulkdate']."'";
...
else if(isset($_POST['action']) && $_POST['action'] == 'conditionalreportform')
{
    ...
    $image_parts1 = explode(";base64,", $_POST['sig1']);
