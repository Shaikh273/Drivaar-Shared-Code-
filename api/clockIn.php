...
$inspection_date=$_POST['starts'];
$description=$_POST['description'];
$date=date('Y-m-d');
$status=0;
$todaydate = date('Y-m-d H:i:s');
if(isset($auth_key) && isset($clockin_status) && $clockin_status=='clockin' && isset($inspection_date))
{
            ...
            if($driverresult>0)
            {
                ...
                if($insresult>0)
                {
                   // clockin & clockout
                	...
                	$valus[0]['contractor_id'] =$row['id'];
                    $valus[0]['starts'] = $inspection_date;
                    $valus[0]['description'] = $description;
                    $valus[0]['date'] = date('Y-m-d');
    				$valus[0]['insert_date'] = date('Y-m-d H:i:s');
			        $makeinsert1 = $mysql -> insert('tbl_contractorattendance',$valus);

			        //$makecol = array('clockin','clockout');
                    //$where = 'id ='. $row['id'];
                ...
                {
                    $status=0; 
                    $error = "Driver has no inspection.";
                    $errorcode = "109";
                }
            }
            else
